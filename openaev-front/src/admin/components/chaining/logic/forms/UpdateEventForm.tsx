...
import type { ConditionRow } from './CreateEventForm';
...
const UpdateEventForm: FunctionComponent<Props> = ({
  initialName,
  initialDescription,
  initialRootLogicalType,
  initialConditions,
  onSubmit,
  handleClose,
}) => {
  ...
  const [rootLogicalType, setRootLogicalType] = useState<string>(initialRootLogicalType);
  const [conditions, setConditions] = useState<ConditionRow[]>(
    initialConditions.length > 0 ? initialConditions : [emptyCondition()],
  );
